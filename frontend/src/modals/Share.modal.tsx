...
const ShareModal: React.FC<ShareProps> = ({ document, setShowShareModal }) => {
    ...
    try {
      ...
      if (response.data.data.isPublic) {
        ...
        dispatch(
          setShowToast({
            showToast: true,
            message: 'Document is no longer public',
            type: 'SUCCESS',
            timing: 3000,
          }),
        );
    ...
    try {
      ...
      if (accessType === 'read') {
        ...
        dispatch(
          setShowToast({
            showToast: true,
            message: 'Public access is set to view',
            type: 'SUCCESS',
            timing: 3000,
          }),
        );
        ...
        dispatch(
          setShowToast({
            showToast: true,
            message: 'Public access is set to edit',
            type: 'SUCCESS',
            timing: 3000,
          }),
        );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent): void => {
      if (isPublicRef.current && !isPublicRef.current.contains(event.target as Node))
        setShowPublicAccessModal(false);
      if (publicAccessRef.current && !publicAccessRef.current.contains(event.target as Node))
        setShowPublicAccessTypeModal(false);
    };
  ...
  return (
    <div className="WAPPER fixed top-0 left-0 w-screen h-screen flex justify-center items-center bg-gray-300 bg-opacity-50 backdrop-blur-sm text-black font-light select-none">
      <div className="MAIN-CONTAINER flex flex-col justify-center gap-6 py-6 bg-white w-[35vw] h-auto rounded-lg">
        {isLoading ? (
          ...
          <>
              ...
              <div className="relative OWNER flex items-center gap-2 w-full h-14 px-6 cursor-pointer hover:bg-primaryExtraLight">
                ...
                <div className="flex flex-col items-start text-sm font-medium">
                  <span className="relative flex items-center" ref={isPublicRef}>
                    ...
                    {showCircularLoader ? (
                      <span>
                        <CircularLoader size="size-5" border="border border-[3px]" />
                      </span>
                    ) : null}
                  </span>
                  <span className="text-xs font-light ml-2">
                    {isPublic?.description} {isPublic.tag !== 'Restricted' ? accessType : null}
                  </span>
                </div>

                <button
                  className={`absolute flex items-center gap-1 right-6 text-sm`}
                  ref={publicAccessRef}
                >
                  <span
            ...
            <div className="BUTTONS flex justify-between items-center px-6">
              ...
              <button
                className="DONE w-36 h-10 bg-primary hover:bg-primaryDark text-sm text-white font-medium rounded-3xl"
